...
## Active Thread
_Checkpoint: 2026-07-04T03:00:00Z_

**In progress:** forge/ v4 (variability + detail) shipped and headless-verified; craft gate on v4 is OPEN — tunnel live, awaiting Usul's verdict.
**Context:** v3 verdict PASSED ("proper color gradings and texturization, several steps above minecraft — not close to endgame yet"). Usul's v4 ask: variability (metal types, hilt variety) + detail (engravings, wood texture, wear levels). Parallelize-task verdict: SERIAL (29% savings sub-bar, one shader string, DNA order contention); executed as one DNA-contract commit + 6 serial features. Shipped: 5 DNA appends at indices 39-43 (metal_family steel×3/iron/bronze · grip_material leather×2/cord/wire · guard_ext inherit×2/swept/ring · engrave_on 25% · engrave_density) — 12/12 node tests incl. new B10 (v1+v2 golden fixtures survive), B11, B12. Shader: family color/F0 (FAMILY_COLOR in JS + metalness per family, bronze antiques instead of bluing), wood grain (albedo fbm + 0.85 micro-normal amp), 3 grip materials (leather/helical cord/wound wire), swept knuckle-bow + ring guards (SDF, guard_ext overrides guard_type), edge nicks (SDF bites, wear>0.55, both classes), iron rust + bronze verdigris (fbm blotches keyed to wear+grime), engraved inscription bands (hashed tilted strokes + border rails, blade flat / axe cheek). Gotcha fixed: `patch` is a GLSL reserved word. browser-verify PASS 1-6 (console clean all variants; bronze warmRatio 1.478; blued cool-shifted; iron+wear darker/warmer), check 7 partial (spp caption stalls at 30 in their 4s poll window — likely just slow native-res accumulation, not a regression; worth one look on real hardware).
**Next action:** Usul reviews v4 at https://impossible-suits-updates-regarding.trycloudflare.com (hard-refresh). Review hashes: #d740b6e3 (steel sword, swept guard, cord grip) · #8833f287 (bronze damascus, engraved, wear .63) · #b24e896d (iron blued, wire grip, ring guard, engraved) · #dc97372e (bronze damascus axe) · #00c0ffee (iron axe, wear .77, wood grain). On feedback: tune named constants (FAMILY_COLOR, rust/verdigris colors in surfaceMat, engrave depths). On PASS: close iteration + decide push (repo local-only).
**Suggested skills:** none — direct craft loop.
**Files in flight:** none (all committed through the v4 slice; `git -C ~/sketches log --oneline -8` shows the chain)
